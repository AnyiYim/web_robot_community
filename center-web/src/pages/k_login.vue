<template>
  <div style="background-image: url(../public/img/lll.jpg)">
    ...
    <div class="flex flex-center" style="margin: 7% auto 20px auto;position: relative;">
      <Card class="form" style="position: relative">
        <div class="title flex flex-pack-justify">
          <span style="margin-top: -15px;font-size: 20px; color: #000; font-weight: bold">安保端</span>
          <div v-if="!signflag">
            用户登录
          </div>
          <div v-else>
      ...
      login: function () {
        ...
        if (this.login_form.user && this.login_form.pw) {
          this.ajax.post('/admin/login',{
            ...
            if (response.data.code===0 && response.data.data.role === 'ROLE'){
